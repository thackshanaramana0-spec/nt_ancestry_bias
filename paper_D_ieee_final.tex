...
foundation-model scoring. Addressing this asymmetry will require architecturally reference-agnostic
training---such as pangenome-based representations---rather than simply
expanding training-genome diversity within the current linear-reference
framework.
\end{abstract}
...
gnomAD common variants and complete population allele frequency data; results on
this chromosome are broadly representative of autosomal patterns. Two variant
...
where a population-fair model should score near zero.
...
Examining each group separately, AFR variants show a modest absolute reduction
in mean bias (12.6\%, $p{=}0.041$, $d{=}0.033$), but EUR variants do not
(6.8\%, $p{=}0.635$). Because EUR bias falls at the same rate as AFR bias, the
relative asymmetry between groups is preserved.
