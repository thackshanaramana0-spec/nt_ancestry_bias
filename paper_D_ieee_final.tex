...
These results are highly significant statistically yet modest in absolute magnitude:
...
The Mann-Whitney AUROC for classifying a variant as EUR-enriched by bias score
alone is 0.562 (NT-hg38; $p{=}1.3{\times}10^{-6}$) and 0.557 (NT-1000G;
$p{=}8.0{\times}10^{-6}$). The modest AUROC reflects high within-population
scoring variance ($\sigma{\approx}2.23$): bias is systematic at the population
level but not deterministic per variant. Cohen's $d{=}0.19$ (small by the
conventional threshold of 0.20) should be read alongside the resampling
and ranking displacement results below, which translate this per-variant gap into
population-scale consequences.
